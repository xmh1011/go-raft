...
const (
	...
	electionTimeout   = 500 * time.Millisecond // 选举超时时间
)
...
func (r *Raft) getLastLogInfoForElection() (lastLogIndex uint64, lastLogTerm uint64, err error) {
	...
	if lastLogIndex > 0 {
		lastEntry, err := r.store.GetEntry(lastLogIndex)
		if err != nil || lastEntry == nil {
			log.Printf("[ERROR] Node %d failed to get last log entry for election: %v", r.id, err)
			return 0, 0, err
		}
