...
func (r *Raft) TakeSnapshot(logSizeThreshold int) {
	...
	// 3. 将快照持久化并压缩日志。
	if err := r.persistSnapshotAndCompactLog(snapshot); err != nil {
		// 错误已在辅助函数中记录，这里直接返回。
		return
	}
...
// persistSnapshotAndCompactLog 负责将快照保存到稳定存储，并根据快照索引压缩日志。
// 此函数必须在持有锁的情况下被调用。
func (r *Raft) persistSnapshotAndCompactLog(snapshot *param.Snapshot) error {
	// 将快照持久化到存储。
...
// sendSnapshot 是 Leader 用于向落后的 Follower 发送快照的新函数。
